...
main = do insts <- (map read . lines) <$> getContents
          ...
          putStrLn . show . getSum . snd . execRWS run (listArray (0, len) insts) $ (0, M.fromDistinctAscList . zip [0..len] . repeat $ False)
...
step :: RWS (Array Int Instruction) (Sum Int) (Int, M.IntMap Bool) Status
step = do
    (p, e) <- get
    insts <- ask
    if | p > (snd . bounds $ insts) -> return Terminated
       | e M.! p -> return Loop
       | otherwise -> do
           put (p, M.adjust (const True) p e)
           exec $ insts ! p
           return Running

run :: RWS (Array Int Instruction) (Sum Int) (Int, M.IntMap Bool) Status
